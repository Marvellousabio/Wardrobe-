      ...
      {generatedOutfits.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className={darkMode ? "text-gray-400" : "text-gray-600"}>
            No outfits generated yet. Add events and wardrobe items to get
            started.
          </p>
        </div>
      ) : (
          ...
          {generatedOutfits.map((outfit) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${darkMode ? "bg-blue-600" : "bg-blue-500"} text-white`}
                  >
                    {outfit.occasion}
                  </span>
                </div>
              ...
              <div className="space-y-3">
                {outfit.items.map((item, idx) => (
                  <div
                    key={idx}
                    className={`${darkMode ? "bg-gray-600" : "bg-white"} p-3 rounded-lg flex items-center gap-3`}
                  >
