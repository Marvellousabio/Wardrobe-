...
const WardrobeApp = () => {
    ...
    if (file && user) {
      ...
      try {
        ...
        const storageRef = ref(
          storage,
          `users/${user.uid}/wardrobe/${Date.now()}_${file.name}`,
        );
  ...
  const generateOutfits = (occasion: string, weather: string): Outfit[] => {
    const shirts = wardrobe.filter(
      (item) => item.type === "shirt" || item.type === "dress",
    );
    const bottoms = wardrobe.filter((item) => item.type === "pants");
    const layers = wardrobe.filter(
      (item) => item.type === "jacket" || item.type === "sweater",
    );
  ...
  const handleImportData = (importedData: any) => {
    ...
    if (importedData.favorites)
      updateData({ favorites: importedData.favorites });
  };
  ...
  return (
    <>
      ...
      <div
        className={`min-h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-gradient-to-br from-blue-50 to-purple-50 text-gray-900"} p-6`}
      >
        <div className="max-w-6xl mx-auto">
          ...
          <div
            className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-xl p-8 min-h-96`}
          >
            {activeTab === "scanner" && (
              <ScannerTab
                scanning={scanning}
